package com.plectix.simulator.subViews.UtilsForParserXML;
...
public class Tag {
	...
	public boolean equals(Object aTag){
		...
		if(getClass() != aTag.getClass()) return false;
	...
	private boolean equalsWithThis(Tag tag) {
		...
		if(getDataList().size() != tag.getDataList().size()) return false;
		
		for(String dataComponent : getDataList()) {
			if(!tag.getDataList().contains(dataComponent)) return false;
		}
