...
public final class JavaXMLMaker {
	...
	private final double[] time = new double[] { 0.01, 1000, 0.5, 100,
	...
	private void setup(SimulationService service, String fileName, double time) throws Exception {
		SimulatorCommandLine commandLine = new SimulatorCommandLine(
				new String[] { "--sim", mySourcePath + fileName, "--time", "" + time });

		commandLine.getSimulationArguments().setXmlSessionName(
				myOutputPath + fileName.substring(0, fileName.indexOf(".ka")) + "_java.xml");
