...
public class FullereneCreator {
	...
	private static final String TEST_DATA_DIRECTORY = "test.data"+ File.separator + "smiles" + File.separator + "fullerenes-ccd1";
	private static final String OUTPUT_FILENAME = "Fullerenes.txt";
	
	private static final String[] AGENT_NAMES = {"A", "B", "C", "D", "E", "F"};
	private static final String[] SITE_NAMES = {"x", "y", "z", "t", "q", "r", "p", "m", "h", "g", "f", "s", "w", "k", "j", "n", "b", "c", "a", "d"};
	
	public static void main(String[] args) throws IOException {
		List<String> ccd1Files = crawlFolder(TEST_DATA_DIRECTORY, new ArrayList<String>());
		PrintStream outputStream = new PrintStream(new FileOutputStream(OUTPUT_FILENAME));
		
		// System.err.println("Found files: " + ccd1Files.size());
		int counter = 0;
	...
	private static final String createFullerene(BufferedReader reader) throws IOException {
		String line = reader.readLine();
		int numberOfVertices = Integer.parseInt(line.trim());
		List<FullereneVertex> vertices = new ArrayList<FullereneVertex>(numberOfVertices);
		...
		if (vertices.size() != numberOfVertices) {
			throw new RuntimeException("Unexpected number of vertices: " + vertices.size() + " != " + numberOfVertices); 
		}
	...
	private static final List<String> crawlFolder(String testDataDirectory, List<String> ccd1Files) {
		...
		if (file.isFile()) {
			...
			throw new RuntimeException("Don't know how to deal with file " + file.getName());
		}
	...
	private static final class FullereneVertex {
		...
		public FullereneVertex(String agentName, String line) {
			...
			for (String field : fields) {
				if (field.length() > 0) {
					// System.err.println("|" + field.trim() + "|" + " where " + line);
					data[i++] = field.trim();
				}
			}
		...
		public final int computeKappaString(List<FullereneVertex> vertices, int lastLinkIndex) {
			...
			for (int i = 0; i < agentLinks.length; i++) {
				...
				if (linkIndex == 0) {
					...
					vertices.get(neighbors[i]-1).setLinkIndex(this, linkIndex);
				} 
				kappaStringBuffer.append(SITE_NAMES[i] + "!" + linkIndex + ",");
			}
			
			kappaStringBuffer.replace(kappaStringBuffer.length()-1, kappaStringBuffer.length(), ")");
			kappaString = kappaStringBuffer.toString();
			return lastLinkIndex;
		}

		private final void setLinkIndex(FullereneVertex fullereneVertex, int linkIndex) {
		...
		public final void sanityCheckNeighbors(List<FullereneVertex> vertices) {
			List<FullereneVertex> verticesCopy = new ArrayList<FullereneVertex>(vertices);
			...
			for (int i= 0; i < neighbors.length; i++) {
				...
				if (!found) {
					...
					throw new RuntimeException("sanityCheckNeighbors failed for vertix: " + id 
							+ " (" + x + ", " + y + ", " + z + ") [" 
							+ neighbors[0] + " " + neighbors[1] + " " + neighbors[2] + "] [" 
							+ verticesCopy.get(0).id + " " + verticesCopy.get(1).id + " " + verticesCopy.get(2).id + "]\n"
							+ sb.toString());
				}
			}
		}
