...
class MockAction {
	...
	private List<UMockCorrelationAgent> correlationAgents;
	 ...
	 */
	public MockAction(MockRule rule) {
		this.rule = rule;
		this.agentsToAdd = new ArrayList<CAbstractAgent>();
		correlationAgents = new ArrayList<UMockCorrelationAgent>();
...
//		boolean[] checking = new boolean[lhs.size()];
		for (CAbstractAgent lhsAgent : lhs) {
			...
			if (isFit(lhsAgent, rhsAgent)) {
				UMockCorrelationAgent ua = new UMockCorrelationAgent(
	...
	private void addAgentToDelete(CAbstractAgent agentIn) {
		UMockCorrelationAgent ua = new UMockCorrelationAgent(this,
				agentIn, null);
	...
	public List<CAbstractAgent> apply(
			List<UMockCorrelationAgent> injList,
			CMockSubViews solution, List<String> addListString) {
		// TODO apply
		...
		int i = 0;
		for (UMockCorrelationAgent corLHSandRHS : correlationAgents) {
			UMockCorrelationAgent corLHSandSolution = injList.get(i);
