 ...
 *
 */
class MockRule extends SubViewsRule{

	private CMockSubViews solution;
	private List<CAbstractAgent> lhsAgents;
	private List<CAbstractAgent> rhsAgents;
	private MockAction abstractAction;
	private CRule rule;
	 ...
	 */
	public MockRule(CMockSubViews solution,
			CRule rule) {
		super(rule);
	...
	public List<CAbstractAgent> getNewData() {
		...
		if (lhsAgents.size() == 0) {
			newData.addAll(abstractAction.apply(
					new ArrayList<UMockCorrelationAgent>(), solution,
					addListString));
			return newData;
		}
		...
		while (!isEnd(indexList, maxIndex)) {
			List<UMockCorrelationAgent> injList = createInjectionList(
	 ...
	 */
	private List<UMockCorrelationAgent> createInjectionList(
		...
		List<UMockCorrelationAgent> list = UMockCorrelationAgent
