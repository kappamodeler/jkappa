...
//							constraintLeftToRight, myData.getRuleCounter(), 
...
//								constraintRightToLeft, myData.getRuleCounter(), 
//								myArguments.isStorify()));
				...
				case CC_RHS: {
					...
					rules.add(BuildUtil.buildRule(left, right, name,
							activity, ruleID, myArguments.isStorify()));
					if (typeRule == RULE_TWO_WAY) {
						rules.add(BuildUtil.buildRule(
								parseAgent(rhs.trim()), left, nameOp, activity2,
								ruleID, storify));
					}
					break;
				}
				case CC_ALL: {
					...
					if (typeRule == RULE_TWO_WAY) {
						rules.add(BuildUtil.buildRule(
								parseAgent(rhs.trim()), parseAgent(lhs.trim()), nameOp,
								activity2, ruleID, storify));
					}
					break;
				}
				}
...
//			int index, double constraintLeftToRight,
//			double constraintRightToLeft) throws ParseErrorException {
//		if (index == -1)
